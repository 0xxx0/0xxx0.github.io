(function(root,factory){
  ...
  'use strict';
  const SCHEMA='0xxx0/lens-state/v0.1';
  const DESCRIPTOR_SCHEMA='0xxx0/lens-descriptor/v0.1';
  ...
  function globMatch(pattern,value){
    ...
    const safe=pattern.split('*').map(x=>x.replace(/[.*+?^${}()|[\]\\]/g,'\\  function validate(input){
    const state=normalize(input),errors=[];')).join('.*');
    try{return new RegExp('^'+safe+'
  ...
  function fromFieldRoute(route={},options={}){
    ...
    return normalize({
      ...
      sourceRefs:[href],
      provenance:{source:'field-index-map',index:route.index||null},
      meta:{contract:'field-route/v0.1',title:route.title||href,state:route.state||null,operation:route.operation||null,family:route.family||null,role:route.role||null}
    });
  }
  ...
  function fromLegacyHandoff(payload={}){
    ...
    if(payload?.schema==='scale-lens.handoff/v1'){
      return normalize({
        ...
        meta:{title:payload.title||'',at:payload.at||null}
      });
    }
  ...
  return Object.freeze({
    ...
    globMatch,currentContract,supportDescriptor,supportCatalog,compose,uncompose,withFocus,withAperture,withProjection,
    fromFieldRoute,fromScaleSnapshot,fromLegacyHandoff,makeHandoff,storeHandoff,readHandoff
  });
});
).test(value)}catch(_){return pattern===value}
  }
  ...
  function supportDescriptor(input,descriptor){
    ...
    if(!v.ok)return{supported:false,support:0,reasons:v.errors.slice(),descriptor:v.descriptor,currentContract:currentContract(state),nextContract:currentContract(state)};
    const d=v.descriptor,current=currentContract(state),required=text(d.inputContract,'*');
  ...
  function compose(input,descriptor){
    const state=normalize(input),v=validateDescriptor(descriptor);
    if(!v.ok)throw new Error(v.errors.join('; '));
    state.lensStack=[...state.lensStack,v.descriptor];
    state.operator='COMPOSE';
    return validate(state).state;
  }
  ...
  function fromFieldRoute(route={},options={}){
    ...
    return normalize({
      ...
      sourceRefs:[href],
      provenance:{source:'field-index-map',index:route.index||null},
      meta:{title:route.title||href,state:route.state||null,operation:route.operation||null,family:route.family||null,role:route.role||null}
    });
  }
  ...
  function fromScaleSnapshot(snapshot={},options={}){
    ...
    return normalize({
      ...
      sourceRefs:[snapshot.source?.id,snapshot.source?.name,snapshot.source?.originUrl].filter(Boolean),
      provenance:{source:snapshot.source||null,chain:snapshot.chain||[]},
      meta:{scope:snapshot.scope??null,scopeName:snapshot.scopeName??null}
    });
  }
  ...
  return Object.freeze({
    SCHEMA,DESCRIPTOR_SCHEMA,HANDOFF_SCHEMA,HANDOFF_KEY,VIEW,ACTION,
    normalize,validate,normalizeDescriptor,validateDescriptor,serialize,deserialize,equivalent,
    compose,uncompose,withFocus,withAperture,withProjection,
    fromFieldRoute,fromScaleSnapshot,fromLegacyHandoff,makeHandoff,storeHandoff,readHandoff
  });
});
