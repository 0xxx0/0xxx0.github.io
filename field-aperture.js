...
class FieldAperture extends HTMLElement{
 constructor(){super();this.attachShadow({mode:'open'});this.A=null;this.scale=0;this.pos=0;this.timer=null;this.wpm=300;this.speaking=false;this.bound=this.onPointer.bind(this)}
 connectedCallback(){this.renderShell();if(this.hasAttribute('source'))this.load(this.getAttribute('source'),{label:this.getAttribute('label')||'Source'})}
 disconnectedCallback(){this.stop()}
 load(source,opt={}){this.stop();this.A=analyze(source,opt.label||'Untitled');this.scale=clamp(opt.scale??0,0,this.A.scales.length-1);this.pos=0;this.render()}
