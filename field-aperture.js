...
class FieldAperture extends HTMLElement{
 constructor(){super();this.attachShadow({mode:'open'});this.A=null;this.scale=0;this.pos=0;this.anchor=0;this.jsonPath=null;this.voiceCursor=null;this.timer=null;this.rsvp=false;this.wpm=300;this.speaking=false;this.voiceToken=0;this.structureDomKey='';this.bound=this.onPointer.bind(this);this.boundKey=this.onKey.bind(this);this.ratePresets=[120,200,300,450,650,900,1200,1600,2200,3000]}
 ...
 *{box-sizing:border-box}button{font:inherit;border-radius:0}.ap{display:grid;grid-template-columns:auto minmax(0,1fr);gap:9px;border:1px solid var(--fa-line);background:var(--fa-bg);padding:7px;min-height:142px}
 ...
 render(){
  ...
  const markerKey=(this.A.kind||'')+'|'+st.markers.slice(0,40).map(m=>Math.round(clamp(m.at,0,1)*10000)+':'+(m.path||m.label||'')).join('|');
  if(markerKey!==this.structureDomKey){const ticks=q('structureTicks'),marks=q('mapMarks');ticks.replaceChildren();marks.replaceChildren();for(const m of st.markers.slice(0,40)){const line=document.createElementNS(NS,'line'),ma=-Math.PI/2+clamp(m.at,0,1)*Math.PI*2;line.setAttribute('class','structureTick');this.setLine(line,46,ma,5);ticks.appendChild(line);const x=document.createElement('i');x.className='mapMark';x.style.left=(clamp(m.at,0,1)*100)+'%';x.title=m.label||m.path||'';marks.appendChild(x)}this.structureDomKey=markerKey}
