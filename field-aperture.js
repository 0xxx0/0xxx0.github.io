...
class FieldAperture extends HTMLElement{
 ...
 snapshot(){const c=this.current(),s=this.currentScale();return{schema:'field-aperture-focus/v0.2',kind:this.A?.kind,label:this.A?.label,bytes:this.A?.bytes,scale:s?.id,scale_label:s?.label,index:this.pos,count:s?.units.length,address:c?.path||null,focus:typeof c?.value==='string'?c.value:preview(c?.value,240),wpm:this.wpm,material:this.material()}}
 magnitude(){
 ...
 *{box-sizing:border-box}button{font:inherit;border-radius:0}.ap{display:grid;grid-template-columns:auto minmax(0,1fr);gap:9px;border:1px solid var(--fa-line);background:var(--fa-bg);padding:7px;min-height:126px}
 ...
 scheduleRSVP(){if(!this.A)return;this.timer=setTimeout(()=>{this.timer=null;this.step(1);if(this.A)this.scheduleRSVP()},this.dwell());this.render()}
 toggleRSVP(){if(this.timer){clearTimeout(this.timer);this.timer=null;this.render();return}let s=this.currentScale();if(!s)return;if(s.units.length<2){const preferred=this.A.kind==='TEXT'?this.A.scales.findIndex(x=>x.id==='WORD'):this.A.scales.findIndex(x=>x.units.length>1);if(preferred>=0){this.scale=preferred;this.pos=0;s=this.currentScale()}}if(!s||s.units.length<2)return;this.scheduleRSVP()}
 stop(){if(this.timer){clearTimeout(this.timer);this.timer=null}if('speechSynthesis'in window)window.speechSynthesis.cancel();this.speaking=false;this.render()}
 speak(){
