...
const rgba=(hex,a=1)=>{
  ...
  return `rgba(${(n>>16)&255},${(n>>8)&255},${n&255},${clamp(Number(a)||0,0,1)})`;
};
...
export class Renderer {
  ...
  _dropPulse(now){
    ...
    for(const b of this.dropBursts){
      const q=clamp((now-b.at)/1900,0,1),s=clamp((Number(b.strength)||0)*(this.profile?.dropGain||1),0,1.65);
  ...
  _background(state,t){
    ...
    const pg=world.patternGain*imm;
    if(world.pattern==='TRANCE'||world.pattern==='LASER'){
      g.strokeStyle=rgba(world.accent,.045*pg);g.lineWidth=.8;
      for(let i=-5;i<=5;i++){g.beginPath();g.moveTo(i*w*.06,h*.22);g.lineTo(i*w*.22,h*.78);g.stroke()}
    }else if(world.pattern==='GRAIN'){
      g.strokeStyle=rgba(world.accent,.038*pg);g.lineWidth=.7;
      for(let i=0;i<8;i++){const rr=this.r*(.28+i*.115)+Math.sin(t*.00022+i*1.7)*4;g.beginPath();g.ellipse(0,0,rr,rr*(.48+.05*Math.sin(i)),.12*Math.sin(i*.9),0,TAU);g.stroke()}
    }else if(world.pattern==='VOID'){
      for(let i=0;i<18;i++){const a=(i*.61803398875+t*.000006)%1*TAU,r=this.r*(.42+((i*37)%17)/15);g.fillStyle=rgba(world.accent,.018+.025*((i%5)/4));g.beginPath();g.arc(Math.cos(a)*r,Math.sin(a)*r*.72,.6+(i%3)*.45,0,TAU);g.fill()}
    }else{
      for(let i=0;i<5;i++){g.strokeStyle=rgba(world.accent,.014+i*.006);g.lineWidth=.7;g.beginPath();g.arc(0,0,this.r*(.34+i*.17)+Math.sin(t*.0003+i)*2,0,TAU);g.stroke()}
    }
    const beatLife=clamp(1-(t-this.beatAt)/260,0,1);
    if(beatLife>0){g.strokeStyle=rgba(world.accent,clamp((.12+.38*this.beatEnergy)*beatLife*imm,0,.72));g.lineWidth=1+2.4*beatLife;g.beginPath();g.arc(0,0,this.r+10+16*(1-beatLife),0,TAU);g.stroke()}
    g.restore()
  }
  _pov(state,t){
    ...
    for(const b of this.dropBursts){
      const q=clamp((t-b.at)/1900,0,1),life=Math.sin(Math.PI*q),strength=clamp((Number(b.strength)||0)*(this.profile?.dropGain||1),0,1.35);
      if(life<=0)continue;
  ...
  _trackfield(state,t){
    ...
    haze.addColorStop(0,rgba(worldStyle.accent,clamp((.07+.12*clamp(proj.current?.brightness||0,0,1))*worldStyle.horizonGain*profile.immersion,0,.28)));
    ...
    const roadColor=(p)=>{
      ...
      return p.sectionIndex%2===0?worldStyle.roadA:worldStyle.roadB;
    };
    const fillQuad=(a,b,sign=0,opacity=1)=>{
      ...
      const op=clamp(opacity,0,1),solid=clamp(worldStyle.solidBase*profile.solidity*(.96+.04*hot),.68,1);
      const path=()=>{g.beginPath();g.moveTo(aCenter-aHalf,a.baseY);g.lineTo(aCenter+aHalf,a.baseY);g.lineTo(bCenter+bHalf,b.baseY);g.lineTo(bCenter-bHalf,b.baseY);g.closePath()};
      // Geometry is matter: an opaque under-surface provides real occlusion before atmospheric color.
      g.globalCompositeOperation='source-over';g.globalAlpha=clamp((.82+.18*op)*solid,.60,1);g.fillStyle=worldStyle.side;path();g.fill();
      g.globalAlpha=clamp((.54+.26*hot+.08*flux)*op*solid,.16,.94);g.fillStyle=roadColor(a);path();g.fill();
      g.globalAlpha=1;
    };
    ...
    for(let i=s.length-2;i>=0;i--){
      ...
      if(mix>.005){
        const leftOpacity=mix*(rideChoice===1?.42:1);
        const rightOpacity=mix*(rideChoice===-1?.42:1);
    ...
    if(splitMix>.005){
      for(const sign of [-1,1]){
        ...
        g.strokeStyle=rgba(worldStyle.edge,alpha);g.lineWidth=chosen&&rideChoice?1.8:1;
  ...
  _bitArc(g,r,a0,a1,bit){
    ...
    g.beginPath();
    if(Number(bit)===1)g.arc(0,0,r,a0,a1);
    else{g.arc(0,0,r,a0,mid-gap);g.moveTo(Math.cos(mid+gap)*r,Math.sin(mid+gap)*r);g.arc(0,0,r,mid+gap,a1)}
    g.stroke();
  }
  _gameProjection(state,t){
    const v=this.gameProjection;if(!v?.active)return;
    const g=this.g,mode=v.mode,r=this.r;g.save();g.translate(this.cx,this.cy);g.lineCap='round';
    if(mode==='PLAY'){
      const n=8,releases=Math.max(0,Number(v.releases)||0),hits=Math.max(0,Number(v.hits)||0),rr=r+14;
      for(let i=0;i<n;i++){const a=-Math.PI/2+i*TAU/n;g.strokeStyle=i<releases?(i<hits?'rgba(114,228,182,.82)':'rgba(255,255,255,.42)'):'rgba(255,255,255,.10)';g.lineWidth=i<releases?2.3:1;g.beginPath();g.arc(0,0,rr,a-.07,a+.07);g.stroke()}
    }else if(mode==='PATH'){
      const n=5,round=Math.max(0,Math.min(n,(Number(v.path?.round)||1)-1)),rr=r*.76;
      ...
      for(const [rr,col,label] of [[ro,'rgba(123,213,255,.42)','A'],[ri,'rgba(239,120,73,.48)','B']]){
        g.strokeStyle=col;g.lineWidth=2;g.beginPath();g.arc(0,0,rr,0,TAU);g.stroke();
        for(let i=0;i<6;i++){const ang=-Math.PI/2+i*TAU/6;g.strokeStyle=col;g.lineWidth=1.2;g.beginPath();g.moveTo(Math.cos(ang)*(rr-7),Math.sin(ang)*(rr-7));g.lineTo(Math.cos(ang)*(rr+7),Math.sin(ang)*(rr+7));g.stroke()}
        g.fillStyle=col;g.font='900 8px ui-monospace,monospace';g.textAlign='left';g.fillText(label,rr*.70,-rr*.70);
      }
      const aa=-Math.PI/2+a*TAU/6,bb=-Math.PI/2+b*TAU/6,ax=Math.cos(aa)*ro,ay=Math.sin(aa)*ro,bx=Math.cos(bb)*ri,by=Math.sin(bb)*ri;
      g.strokeStyle='rgba(255,255,255,.58)';g.lineWidth=1.8;g.beginPath();g.moveTo(ax,ay);g.lineTo(bx,by);g.stroke();
      g.fillStyle='rgba(123,213,255,.98)';g.beginPath();g.arc(ax,ay,6.5,0,TAU);g.fill();g.fillStyle='rgba(239,120,73,.98)';g.beginPath();g.arc(bx,by,6.5,0,TAU);g.fill();
      g.textAlign='center';g.fillStyle='rgba(255,255,255,.78)';g.font='900 8px ui-monospace,monospace';g.fillText('OUTER A '+a+' × INNER B '+b,0,r*.38);
    }else if(mode==='GARDEN'){
      const gen=Math.max(1,Math.min(3,Number(v.garden?.generation)||1)),events=Math.max(0,Number(v.garden?.events)||0),survived=Math.max(0,Number(v.garden?.survived)||0);
      for(let k=1;k<=3;k++){const rr=r*(.46+k*.105);g.strokeStyle=k<gen?'rgba(114,228,182,.38)':k===gen?'rgba(215,180,109,.72)':'rgba(255,255,255,.08)';g.lineWidth=k===gen?2.4:1.2;g.beginPath();g.arc(0,0,rr,0,TAU);g.stroke()}
      const rr=r*(.46+gen*.105);for(let i=0;i<4;i++){const a=-Math.PI/2+i*TAU/4;g.fillStyle=i<events?'rgba(215,180,109,.9)':'rgba(255,255,255,.12)';g.beginPath();g.arc(Math.cos(a)*rr,Math.sin(a)*rr,3.4,0,TAU);g.fill()}
  ...
  _center(state,t){
    ...
    let stroke=hit?'rgba(255,255,255,.98)':aligned?'rgba(255,255,255,.82)':'rgba(255,255,255,.18)';
    ...
    if(v?.active&&v.mode==='PUZZLE'){
      ...
      g.fillStyle='rgba(255,255,255,.34)';g.font='700 7px ui-monospace,monospace';g.fillText('OUTER ROAD / INNER RELATION',0,31);
    }else if(v?.active&&v.mode==='PATH'){
      g.fillStyle='rgba(255,255,255,.94)';g.font='900 16px ui-monospace,monospace';g.fillText('★ '+(v.path?.stars??0),0,3);
      g.fillStyle='rgba(215,180,109,.82)';g.font='800 8px ui-monospace,monospace';g.fillText('PAR '+(v.path?.par??0)+' · '+(v.path?.turns??0)+'T',0,19);
      g.fillStyle='rgba(255,255,255,.30)';g.font='700 7px ui-monospace,monospace';g.fillText('SHORTEST TURN PATH',0,31);
    }else if(v?.active&&v.mode==='GARDEN'){
      g.fillStyle='rgba(255,255,255,.94)';g.font='900 16px ui-monospace,monospace';g.fillText('G'+(v.garden?.generation??1),0,3);
      g.fillStyle='rgba(114,228,182,.86)';g.font='800 8px ui-monospace,monospace';g.fillText(v.garden?.trait||'OBSERVE',0,19);
      g.fillStyle='rgba(255,255,255,.30)';g.font='700 7px ui-monospace,monospace';g.fillText('INHERIT / SURVIVE / RETURN',0,31);
    }else if(v?.active&&v.mode==='ZEN'){
      g.fillStyle='rgba(255,255,255,.86)';g.font='400 24px ui-monospace,monospace';g.fillText('○',0,5);
      g.fillStyle='rgba(255,255,255,.30)';g.font='700 8px ui-monospace,monospace';g.fillText('FREE',0,22);
    }else{
      g.fillStyle='rgba(255,255,255,.92)';g.font='900 16px ui-monospace,monospace';g.fillText(aligned?(f?.verb||'RELEASE'):'TURN',0,3);
      g.fillStyle=hit?'rgba(255,255,255,.88)':'rgba(255,255,255,.38)';g.font='700 8px ui-monospace,monospace';g.fillText(aligned?(hit?'HIT CALL':`×${f?.chain||1} · OPEN`):`CALL ${callLabel(state.call)}`,0,18);
      g.fillStyle='rgba(255,255,255,.30)';g.font='700 7px ui-monospace,monospace';g.fillText(state.mode==='RATCHET'?`CHARGE ${Math.round(charge/1.75*100)}%`:`FLOW ${state.flow}`,0,31);
    }
    g.restore()
  }
}
