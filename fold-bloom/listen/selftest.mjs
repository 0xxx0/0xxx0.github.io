...
import {scopeWindow} from './audio-map.js';
...
const ok=preview.stage==='PREVIEW'&&preview.frames.length>=24&&preview.duration===dur&&map.bpm>108&&map.bpm<132&&map.beats.length>=20&&map.frames.length>40&&map.sections.length>=2&&scopeOk&&pulseOk;
console.log(JSON.stringify({ok,previewFrames:preview.frames.length,bpm:map.bpm,confidence:map.tempoConfidence,beats:map.beats.length,frames:map.frames.length,sections:map.sections.length,scopeOk,pulseOk,beatWindow,phraseWindow}));
