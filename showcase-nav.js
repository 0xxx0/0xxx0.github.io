...
async function start(){
  ...
  @media(max-width:820px){
    .tab{bottom:max(8px,env(safe-area-inset-bottom));opacity:.82}
    .tab.nav{left:auto;right:max(8px,env(safe-area-inset-right))}
    .tab.read{left:auto;right:max(46px,calc(env(safe-area-inset-right) + 46px))}
    .tab.lens{left:auto;right:max(96px,calc(env(safe-area-inset-right) + 96px))}
    .panel{left:max(8px,env(safe-area-inset-left));right:max(8px,env(safe-area-inset-right));bottom:max(50px,calc(env(safe-area-inset-bottom) + 50px));width:auto;max-height:calc(100dvh - 72px);overflow:auto}
  }
  </style>
  ...
  const readContext=()=>{const sel=String(getSelection?.()||'').trim(),active=document.activeElement;let source='',label=route.title||document.title||p;if(sel){source=sel;label='Selection · '+label}else if(active&&/^(TEXTAREA|INPUT)$/.test(active.tagName)&&String(active.value||'').trim()){source=String(active.value);label=(active.getAttribute('aria-label')||active.name||active.id||'Field')+' · '+label}else{const root=document.querySelector('main')||document.body;source=String(root?.innerText||document.body?.innerText||document.title||'').trim();label='Page · '+label}if(!source)return;source=source.slice(0,160000);const returnAddress=location.pathname+location.search+location.hash;try{sessionStorage.setItem('readfield.handoff.v1',JSON.stringify({schema:'readfield.handoff/v1',source,label,returnAddress,created_at:new Date().toISOString()}))}catch(_){return}go('/docs/?handoff=1&return='+encodeURIComponent(returnAddress))};
