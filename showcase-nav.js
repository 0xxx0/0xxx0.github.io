...
async function start(){
  ...
  <button class="tab nav" aria-label="Open showcase navigation" title="Showcase navigation">↖</button><button class="tab inspect" aria-label="Inspect current FIELD context" title="Inspect selection / field / route">◎</button><button class="tab lens" aria-label="Open Lens" title="Lens / refraction">◎</button>
  ...
  const ensureAperture=()=>new Promise((resolve,reject)=>{if(window.FieldAperture)return resolve(window.FieldAperture);const prior=document.querySelector('script[data-field-aperture-loader]');if(prior){prior.addEventListener('load',()=>resolve(window.FieldAperture),{once:true});prior.addEventListener('error',reject,{once:true});return}const x=document.createElement('script');x.src='/field-aperture.js';x.dataset.fieldApertureLoader='1';x.onload=()=>resolve(window.FieldAperture);x.onerror=reject;document.head.appendChild(x)});
  const inspectContext=async()=>{let source=null,label=route.title||p;const sel=String(getSelection?.()||'').trim();const active=document.activeElement;if(sel){source=sel;label='Selection · '+label}else if(active&&/^(TEXTAREA|INPUT)$/.test(active.tagName)&&String(active.value||'').trim()){source=String(active.value);label=(active.getAttribute('aria-label')||active.name||active.id||'Field')+' · '+label}else{source={href:route.href,title:route.title,kind:route.kind,state:route.state,operation:route.operation,family:route.family,parent:route.parent,role:route.role,index:route.index,receipt:route.receipt};label='Route · '+label}try{const A=await ensureAperture();A?.dock(source,{label,from:p})}catch(e){console.warn('FIELD Aperture unavailable',e)}};
  ...
  inspect.onclick=inspectContext;
