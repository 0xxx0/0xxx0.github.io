...
async function start(){
  ...
  .lenshead{display:flex;justify-content:space-between;gap:8px;padding:8px 10px;border-top:1px solid #29343a;color:#718087;font-size:8px}.lenshead b{color:#eef1ed}.lensread{display:grid;grid-template-columns:72px 1fr;border-top:1px solid #20282c}.lensread span,.lensread b{padding:5px 8px;border-bottom:1px solid #20282c}.lensread span{color:#6f7d83}.lensread b{color:#d9dedd;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.lensops{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:#29343a}.lensops button{border:0;background:#0f1317;color:#eef1ed;padding:8px 5px;font:800 8px ui-monospace,monospace;cursor:pointer}.lensops button:disabled{opacity:.32;cursor:not-allowed}.lensops button:not(:disabled):hover,.lensops button:not(:disabled):focus{background:#172027;outline:none}.lensstack{padding:7px 9px;color:#738087;border-top:1px solid #29343a;font-size:8px;word-break:break-word}.lensstack.live{color:#a9bac1}.lensreturn{color:#d5ad68!important}.lensstudio{color:#72bce7!important}.foot{padding:8px 10px;color:#69757c;font-size:8px;border-top:1px solid #29343a}.family{display:block;border-top:1px solid #29343a;text-align:left;color:#9ba6ac}.meta{display:flex;border-top:1px solid #29343a}.meta a{flex:1;padding:7px 8px;color:#7f8b92;text-decoration:none;font-size:8px;text-align:center}.meta a+a{border-left:1px solid #29343a}.meta a:hover{color:#eef1ed}
  </style>
  ...
  <nav class="panel" aria-label="Showcase route">
    ...
    <div class="lenshead"><b>FOCUS RING</b><span data-l="mode">ROUTE</span></div>
    <div class="lensread"><span>OBJECT</span><b data-l="object">—</b><span>APERTURE</span><b data-l="aperture">—</b><span>VIEW</span><b data-l="view">—</b></div>
    <div class="lensops"><button data-l="out" title="Coarser aperture in live Scale Studio">↑ OUT</button><button data-l="in" title="Finer aperture in live Scale Studio">↓ IN</button><button data-l="projection" title="Cycle lawful projection in live Scale Studio">↔ VIEW</button><button data-l="stackBtn">⊗ STACK</button><button class="lensreturn" data-l="return">↩ RETURN</button><button class="lensstudio" data-l="studio">STUDIO →</button></div>
    <div class="lensstack" data-l="stack">0 · plain baseline</div>
  ...
  const renderLens=()=>{
    ...
    if(s)lensState=s;
    const put=(q,v)=>{const el=lens(q);if(el)el.textContent=v};
    put('mode',a?'LIVE':'ROUTE');put('object',short(s?.objectId||route.id||route.href||p));put('aperture',short(s?.aperture?.level??route.kind??'surface'));put('view',short(s?.projection||route.operation||route.family||'SOURCE'));
    const st=lens('stack');if(st){st.textContent=s?.lensStack?.length?String(s.lensStack.length)+' · '+s.lensStack.map(x=>x.lensId).join(' → '):'0 · plain baseline';st.classList.toggle('live',!!s?.lensStack?.length)}
    if(lens('out'))lens('out').disabled=!a?.aperture;if(lens('in'))lens('in').disabled=!a?.aperture;if(lens('projection'))lens('projection').disabled=!a?.project;
  };
  const openStudio=()=>{
