...
.fl-gate{background:#eee7d9}.fl-detail{display:block}.fl-compact .fl-detail{display:none}.fl-foot{display:flex;justify-content:space-between;gap:8px;align-items:center;padding:6px 9px;border-top:1px solid #191915}.fl-error{padding:8px 9px;color:#a8422d}
...
<div class="fl-detail" id="flDetail"><div class="fl-grid"><div class="fl-col"><div class="fl-label">NOW / ACTIVE FRONTS · EXPLICIT ADD</div><div id="flFronts"></div></div><div class="fl-col"><div class="fl-label">HUMAN GATES · VISIBLE, NEVER AUTO-PROMOTED</div><div id="flGates"></div></div></div>
<div class="fl-foot"><span class="fl-meta">FIELD is read-only here. CAPTURE stays local. COPY HANDOFF emits authority NONE for ChatGPT/Hermes/Codex.</span><a href="/control/CURRENT.json" target="_blank" rel="noopener">SOURCE ↗</a></div></div>`;
...
const toggle=document.getElementById('flToggle');const syncToggle=()=>{if(toggle)toggle.textContent=panel.classList.contains('fl-compact')?'DETAILS':'HIDE'};syncToggle();if(toggle)toggle.onclick=()=>{panel.classList.toggle('fl-compact');syncToggle()};
...
document.getElementById('flFeedback').onclick=async()=>{const b=document.getElementById('flFeedback'),packet=api().handoff?api().handoff():api().feedback(),txt=JSON.stringify(packet,null,2);try{await navigator.clipboard.writeText(txt);b.textContent='COPIED';setTimeout(()=>b.textContent='COPY HANDOFF',1200)}catch{const blob=new Blob([txt],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='atlas-dayline-hermes-handoff.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}};
sync();
})();