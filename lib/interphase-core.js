(function(root,factory){
  'use strict';
  const PATH=(typeof module==='object'&&module.exports)?require('./path.js'):(root?root.InterphasePath:null);
  const api=factory(PATH);
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.Interphase=api;
})(typeof globalThis!=='undefined'?globalThis:this,function(PATH){
  'use strict';
  if(!PATH||typeof PATH.getIn!=='function')throw new Error('INTERPHASE PATH REQUIRED: load /lib/path.js before /lib/interphase-core.js');
  const {clone,getIn,setIn,optic,compose,pathsFor,prop,index,all,where}=PATH;
  ...
  class Host{
    ...
    // TRANSDUCTIONS instrument_cell GLYPH: derived output, never canonical state.
    glyph(ref){
      const r=this._resolve(ref),d=this.adapter.describe(r);
      const out={id:d.id,address:clone(d.address||null),representation:null,support:0,reason:'NO_HOST_GLYPH'};
      if(typeof this.adapter.glyph!=='function')return out;
      try{
        const g=this.adapter.glyph(r);
        if(!g)return out;
        const p=g.recipe;
        if(!p?.lensId||!p?.lensVersion||p.kind!=='VIEW_LENS'||p.authority!=='PREVIEW'||
          !p.inputContract||!p.outputContract||!Array.isArray(p.preserves)||!Array.isArray(p.hides)||!Array.isArray(p.derives)||
          !g.descriptor||typeof g.svg!=='string'||!g.svg.startsWith('<svg')){
          out.reason='INVALID_HOST_GLYPH';return out;
        }
        // Select fields rather than propagating adapter capabilities, effects or identity.
        out.representation=clone({recipe:{lensId:p.lensId,lensVersion:p.lensVersion,kind:p.kind,authority:p.authority,
          inputContract:p.inputContract,outputContract:p.outputContract,params:p.params||{},preserves:p.preserves,hides:p.hides,derives:p.derives},
          descriptor:g.descriptor,svg:g.svg});
        out.support=1;out.reason=null;return out;
      }catch(_){out.reason='HOST_GLYPH_UNAVAILABLE';return out}
    }
    ...
    projectionResult(name=this.state.projection){
      ...
      const projected=name==='GLYPH'?nodes.map(n=>{
        for(const ch of EXPLICIT_PRIMITIVES.map(x=>x.toLowerCase()))if(!residue.some(x=>x.id===n.id&&x.channel===ch))residue.push({id:n.id,channel:ch,projection:name});
        let g;try{g=this.glyph(n.id)}catch(_){g={support:0,reason:'UNRESOLVED',representation:null}}
        return {id:n.id,kind:n.kind,address:clone(n.address||null),channels:['identity','address'],
          glyph:g.representation,glyphSupport:{support:g.support,reason:g.reason},...(n.error?{error:n.error}:{})};
      }):nodes;
      return {schema:VERSION+'/projection',host:this.id,projection:name,spec:clone(spec),nodes:clone(projected),residue,selection:this.state.selection.slice(),focus:clone(this.state.focus),revision:this.state.revision};
    }
