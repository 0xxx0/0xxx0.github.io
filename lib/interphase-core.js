(function(root,factory){
  ...
  class Host{
    ...
    invoke(ref,operation,args={},opt={}){
      ...
      if(out?.ok===false)return out;
      if(stable(before)!==stable(after)||authority==='EFFECT')this.state.revision++;
  ...
  return Object.freeze({VERSION,CHANNELS,DEFAULT_PROJECTIONS,Host,createHost:(adapter,opt)=>new Host(adapter,opt),optic:Object.freeze({prop,index,all,where,compose,getIn,setIn})});
});