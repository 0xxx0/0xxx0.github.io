...
for(const h of current.current_heads||[]){
  const label=h.lineage||h.head||'?';
  if(!h.route){fail.push('current head missing route: '+label);continue}
  ...
  if(cv==null) fail.push('current head missing structured version '+h.route);
...
console.log('CURRENT↔MANIFEST DRIFT PASS · '+seen.size+' current heads · '+(current.active_fronts||[]).length+' active fronts');
