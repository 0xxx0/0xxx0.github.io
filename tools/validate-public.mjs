...
for(const bits of ['000','001','010','011','100','101','110','111'])check(fan8Svg.includes(`class="bitLabel">${bits}</text>`),`FAN/8 visible binary IDs drifted at ${bits}`);
...
const fbAuthorityManifest=parse('showcase-manifest.json');
const fbAuthorityCurrent=parse('control/CURRENT.json');
const fbLiveRelease=parse('fold-bloom/live/release.json');
const fbVoiceRelease=parse('fold-bloom/voice/release.json');
const fbLabRelease=parse('fold-bloom/lab/release.json');
const fbPlayRuntime=read('fold-bloom/live/play.js');
const fbLiveRoute=fbAuthorityManifest?.routes?.find(r=>r.href==='/fold-bloom/live/');
...
check(fbPlayRuntime.includes("const VERSION='FOLD_BLOOM_PLAY_0.5.7'"),'FOLD//BLOOM PLAY runtime version drifted from expected 0.5.7');
check(fbLiveRelease?.play_shell?.version==='0.5.7'&&String(fbLiveRoute?.role||'').includes('PLAY 0.5.7')&&String(fbCurrentHead?.head||'').includes('PLAY 0.5.7'),'PLAY runtime/release/manifest/CURRENT version drift');
check(fbVoiceRelease?.version==='0.2'&&fbVoiceRoute?.version==='0.2'&&fbLabRelease?.version==='0.3.3'&&fbLabRoute?.version==='0.3.3','VOICE/LAB release-manifest version drift');
check(String(fbVoiceRoute?.role||'').includes('FIELD LAB PULSE')&&String(fbLabRoute?.role||'').includes('VOICE 0.2'),'VOICE→PULSE convergence lost from public authority projection');
