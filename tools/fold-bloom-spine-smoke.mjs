...
function probe(){return `<!doctype html><html><body><img id="hold" src="/__hold" hidden><iframe id="f" style="width:430px;height:900px;border:0"></iframe><pre id="probeResult">PENDING</pre><script>
...
setTimeout(()=>{(async()=>{
  const api=await import('/fold-bloom/local-media-store.js'),{hashHex}=await import('/lib/id.js'),blob=wav(),buf=await blob.arrayBuffer(),hash='sha256:'+await hashHex(buf);rec.hash=hash;
  await api.putLocalMedia({sourceId:hash,blob,name:'spine-proof.wav',type:'audio/wav',size:blob.size,meta:{durationSeconds:.5}});rec.stored=true;
...
const server=http.createServer((req,res)=>{
  ...
  if(String(req.url).startsWith('/__spine')){res.writeHead(200,{'content-type':'text/html; charset=utf-8','cache-control':'no-store'});res.end(probe());return}
