...
"const returned=W().CommsSpine.returnObject();await W().CommsSpine.loadSource('User: temporary source','TEMP');rec.temporary=W().CommsSpine.state().sourceId!==rec.sourceId;rec.resumed=W().CommsSpine.loadReturn(returned);const resumed=W().CommsSpine.state();rec.resumeSource=resumed.sourceId===rec.sourceId;rec.resumeDraft=resumed.draft===st.draft;rec.resumeCovered=resumed.signals.some(x=>x.kind==='ASK'&&x.state==='COVERED');"+
"done(rec.messages==='4'&&rec.signals0>0&&rec.signals1>rec.signals0&&rec.covered>=1&&rec.human>=1&&/^sha256:/.test(rec.sourceId)&&rec.draft>10&&rec.packet==='comms-spine-agent-packet/v0.1'&&rec.overflow<=1&&rec.temporary&&rec.resumed&&rec.resumeSource&&rec.resumeDraft&&rec.resumeCovered,rec)})().catch(e=>done(false,{error:String(e&&e.stack||e),...rec}));"+
'</scr'+'ipt></body></html>'}
