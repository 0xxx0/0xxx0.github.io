...
function fieldDaylineHandoffProbeHtml(){
  ...
  (async()=>{
    ...
    rec.task={title:task.title,fieldRef:task.fieldRef,route:task.sourceLink?.source?.route,object:task.sourceLink?.source?.object_id,return_to:task.sourceLink?.return_to,status:task.status};
    rec.cleared=!W().sessionStorage.getItem('atlas.dayline.handoff.v01');
    const ok=rec.fieldFocus==='/docs/'&&rec.path==='/dayline/'&&rec.packet.schema==='atlas-dayline-handoff/v0.1'&&rec.packet.kind==='TASK'&&rec.packet.route==='/docs/'&&rec.packet.object==='route:/docs/'&&rec.packet.return_to==='/'&&rec.before===0&&rec.after===1&&rec.task.fieldRef==='/docs/'&&rec.task.route==='/docs/'&&rec.task.object==='route:/docs/'&&rec.task.return_to==='/'&&rec.task.status==='open'&&rec.cleared;
    done(ok,rec);
...
const CASES=[
  ...
  },
  {
    ...
    check:dom=>/id="probeResult">PASS /.test(dom)&&/"fieldFocus":"\/docs\/"/.test(dom)&&/"schema":"atlas-dayline-handoff\/v0\.1"/.test(dom)&&/"route":"\/docs\/"/.test(dom)&&/"before":0/.test(dom)&&/"after":1/.test(dom)&&/"return_to":"\/"/.test(dom)&&/"cleared":true/.test(dom)
  },
  {
