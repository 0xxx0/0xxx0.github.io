...
function readerFocusProbeHtml(){
  ...
  (async()=>{
    ...
    const perf=Array.from({length:260},(_,i)=>'focus'+i).join(' ');pt.value=perf;pb.click();await waitFor(()=>A.A?.label==='PASTE'&&A.A?.words>=250);A.restore({scale:'WORD',index:0,wpm:3000});
...
function readfieldRouteHandoffProbeHtml(){
  ...
  (async()=>{
    const W=()=>f.contentWindow,D=()=>W().document;
    const sh=await waitFor(()=>D().getElementById('showcase-route-adapter')?.shadowRoot);
    const read=await waitFor(()=>sh.querySelector('.tab.read'));rec.label=read.textContent;read.click();
    await waitFor(()=>W().location.pathname==='/docs/'&&D().getElementById('docAperture')?.A?.label?.startsWith('Page ·'));
