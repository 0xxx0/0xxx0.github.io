...
function readfieldLocalFileProbeHtml(){
  ...
  (async()=>{
    ...
    await W().openLocalFile(file);
    await waitFor(()=>A.A?.label==='field-note.md'&&/field-note\.md · LOCAL/.test(D().getElementById('title')?.textContent||''),3000,'local file loaded');
...
function foldBloomLiveMobileBehaviorProbeHtml(){
  ...
  (async()=>{
    try{
      ...
      const doc=d(),html=doc.documentElement,body=doc.body,settings=doc.getElementById('settings'),menu=doc.getElementById('menuBtn'),dismiss=doc.getElementById('menuDismiss'),close=doc.getElementById('closeSettings'),still=doc.getElementById('fbStillView'),form=doc.getElementById('fbFormPanel'),intro=doc.getElementById('intro'),vibe=doc.getElementById('vibeQuick');
      const formRect=form?.getBoundingClientRect(),release=doc.getElementById('releaseBtn'),releaseRect=release?.getBoundingClientRect();
      rec.initial={mode:w.FoldBloomPlay.state().mode,instrument:html.dataset.fbInstrument,formVisual:html.dataset.fbFormVisual,view:html.dataset.fbView,formOn:form?.classList.contains('on')||false,formDisplay:form?getComputedStyle(form).display:'missing',formRect:formRect?[Math.round(formRect.left),Math.round(formRect.top),Math.round(formRect.width),Math.round(formRect.height)]:null,releaseRect:releaseRect?[Math.round(releaseRect.left),Math.round(releaseRect.top),Math.round(releaseRect.width),Math.round(releaseRect.height)]:null,formClear:!!(formRect&&releaseRect&&formRect.height>0&&formRect.bottom<releaseRect.top-4),introOn:intro?.classList.contains('on')||false,introHidden:!!intro?.hidden};
      ...
      const pass=rec.initial.mode==='PUZZLE'&&rec.initial.instrument==='FORM'&&rec.initial.formVisual==='two-rings'&&rec.initial.view==='ride'&&rec.initial.formOn&&rec.initial.formDisplay!=='none'&&rec.initial.formClear&&!rec.initial.introOn&&rec.initial.introHidden&&rec.open.drawer&&rec.open.html&&rec.open.body&&rec.open.dismiss!=='none'&&/pan-y/.test(rec.open.htmlTouch)&&/pan-y/.test(rec.open.bodyTouch)&&rec.open.topId==='closeSettings'&&rec.drawerControl==='TRANCE'&&rec.backdropClosed&&rec.escapeClosed&&rec.xClosed&&rec.vibe.before!==rec.vibe.after&&rec.still.before==='ride'&&rec.still.mid==='still'&&rec.still.after==='ride'&&rec.overflow<=1;
      done(pass,rec);
...
const CASES=[
  ...
  },
  {
    ...
    check:dom=>/FOLD ?\/\/ ?BLOOM/i.test(dom)&&/A source becomes a field/i.test(dom)&&/ENTER LIVE/.test(dom)&&/TRY AUDIO EXAMPLE/.test(dom)&&!/PLAY CENTER MASS/.test(dom)&&/MAP A TRACK/.test(dom)&&/BUILD A SET/.test(dom)&&/WHAT DO YOU/.test(dom)&&/WANT TO DO/.test(dom)&&/Play a track as terrain/.test(dom)&&/Read fast without losing your place/.test(dom)&&/MORE USES · SABER/.test(dom)&&/SABER \/ TWO PHONES/.test(dom)&&/MESSAGE \/ REPLAY/.test(dom)&&/SURFACES \/ LINEAGE \/ WHY IT WORKS/.test(dom)&&/SOURCE → ADDRESS → TRANSFORM → RETURN/.test(dom)
  },
  {
  ...
  },
  {
    ...
    check:dom=>{
      const parts={
        version:/FIELD LAB 0\.3\.4/i.test(dom),
  ...
  },
  {
    ...
    options:{width:430,height:900,budget:9000,timeout:16000},
    check:dom=>/FIELD LAB 0\.3\.4/i.test(dom)&&dom.includes('data-fold-bloom-field-lab="ready"')&&dom.includes('data-field-lab-mode="VERSE"')&&dom.includes('id="verseSource"')&&dom.includes('id="versePoemMap"')&&dom.includes('id="verseRead"')&&dom.includes('id="verseLoci"')&&dom.includes('id="verseReplay"')&&/MARKS/.test(dom)
  },
  {
    name:'FOLD BLOOM FIELD LAB READFIELD',
    route:'/fold-bloom/lab/?mode=READ',
    options:{width:430,height:900,budget:9000,timeout:16000},
    check:dom=>/FIELD LAB 0\.3\.4/i.test(dom)&&dom.includes('data-fold-bloom-field-lab="ready"')&&dom.includes('data-field-lab-mode="READ"')&&dom.includes('data-field-lab-reader="ready"')&&dom.includes('data-field-lab-reader-scale="WORD"')&&dom.includes('data-field-lab-read-pulse="WITNESS"')&&dom.includes('data-field-lab-read-source="bound"')&&dom.includes('data-field-lab-loci-nodes="13"')&&dom.includes('data-field-lab-loci-words="13"')&&dom.includes('id="labReader"')&&/FULL READFIELD/.test(dom)&&/CARRY → LOCI/.test(dom)&&/CARRY → DATA/.test(dom)&&/EXPORT RETURN/.test(dom)
  },
  {
    name:'FOLD BLOOM FIELD LAB READFIELD PACE4',
    route:'/fold-bloom/lab/?mode=READ&pulse=4',
    options:{width:430,height:900,budget:9000,timeout:16000},
    check:dom=>/FIELD LAB 0\.3\.4/i.test(dom)&&dom.includes('data-fold-bloom-field-lab="ready"')&&dom.includes('data-field-lab-mode="READ"')&&dom.includes('data-field-lab-reader="ready"')&&dom.includes('data-field-lab-read-pulse="PACE4"')&&dom.includes('data-field-lab-read-source="bound"')&&/PULSE · ×4 PACE/.test(dom)
  },
  {
  ...
  },
  {
    ...
    route:'/fold-bloom/live/',
    options:{width:430,height:900,budget:9000},
    check:dom=>/LIVE 0\.13/i.test(dom)&&dom.includes('data-fold-bloom-live="ready"')&&dom.includes('data-fold-bloom-pov="embodied-v0.4"')&&dom.includes('data-fold-bloom-macro-drop="v0.2"')&&dom.includes('data-fold-bloom-idle-law="witness-v0.1"')&&((dom.includes('data-fold-bloom-idle="on"')&&dom.includes('data-fold-bloom-autopilot="on"'))||(dom.includes('data-fold-bloom-idle="off"')&&dom.includes('data-fold-bloom-autopilot="off"')))&&dom.includes('data-fold-bloom-landmarks="0"')&&dom.includes('data-fold-bloom-layer="IMMERSION"')&&dom.includes('id="demoBtn"')&&dom.includes('id="autoBtn"')&&dom.includes('id="publicDemoBtn"')&&/LOAD AUDIO EXAMPLE|PLAY AUDIO EXAMPLE/.test(dom)&&dom.includes('id="centerMassBtn"')&&/TRY CENTER MASS REMOTE/.test(dom)&&/RIDE FIELD COURSE/.test(dom)&&dom.includes('id="vaultSelect"')&&dom.includes('data-layer-mode="SOURCE"')&&dom.includes('data-layer-mode="MAP"')&&dom.includes('data-layer-mode="IMMERSION"')&&/SOURCE → MAP → IMMERSION/i.test(dom)&&dom.includes('id="call"')&&dom.includes('id="arc"')&&dom.includes('id="route"')&&dom.includes('id="trackFile"')&&dom.includes('id="vibeQuick"')&&dom.includes('id="menuDismiss"')&&dom.includes('id="lyric"')&&dom.includes('id="textBtn"')&&dom.includes('id="solidTune"')&&dom.includes('id="immersionTune"')&&dom.includes('id="anticipationTune"')&&dom.includes('id="motionGainTune"')&&dom.includes('id="dropGainTune"')&&dom.includes('id="textSyncTune"')&&dom.includes('data-xp-preset="DRIVE"')&&/data-fold-bloom-ride-profile="[^"]+"/.test(dom)&&/AUTOPILOT|TAKE OVER/.test(dom)&&dom.includes('data-fold-bloom-play="FOLD_BLOOM_PLAY_0.6.0"')&&/RIDE FIELD COURSE/i.test(dom)&&/PLAY MODES · OPTIONAL/i.test(dom)&&/VIBE · NORMAL/i.test(dom)&&/△ TRIANGLE|○ CIRCLE|□ SQUARE/.test(dom)&&/FIELD COURSE/.test(dom)&&dom.includes('data-trackfield-source="FIELD_PRACTICE"')&&/data-trackfield-motion="(?!NONE)[^"]+"/.test(dom)&&/data-fold-bloom-perf="[^"]+"/.test(dom)  },
  {
    name:'FOLD BLOOM LIVE mobile controls clear',
    route:'/fold-bloom/live/?play=PUZZLE',
    options:{width:430,height:900,budget:9000},
    check:dom=>dom.includes('data-fold-bloom-play="FOLD_BLOOM_PLAY_0.6.0"')&&dom.includes('data-fold-bloom-play-loop="FOLD_BLOOM_PLAY_LOOP_0.2"')&&dom.includes('data-fb-play-mode="PUZZLE"')&&dom.includes('data-fb-loop-archetype="FORM_CHANGE"')&&dom.includes('data-fb-instrument="FORM"')&&dom.includes('data-fb-surface="active"')&&dom.includes('data-fb-form-visual="two-rings"')&&dom.includes('data-fb-view="ride"')&&dom.includes('data-fb-primary-controls="clear"')&&!/id="intro"[^>]*class="panel on"/.test(dom)&&/△ TRIANGLE|○ CIRCLE|□ SQUARE/.test(dom)
  },
  {
    ...
    check:dom=>/id="probeResult">PASS /.test(dom)&&/"instrument":"FORM"/.test(dom)&&/"formVisual":"two-rings"/.test(dom)&&/"view":"ride"/.test(dom)&&/"formDisplay":"block"/.test(dom)&&/"formClear":true/.test(dom)&&/"drawerControl":"TRANCE"/.test(dom)&&/"topId":"closeSettings"/.test(dom)&&/"backdropClosed":true/.test(dom)&&/"escapeClosed":true/.test(dom)&&/"xClosed":true/.test(dom)&&/"mid":"still"/.test(dom)&&/"after":"ride"/.test(dom)&&/"overflow":0/.test(dom)
  },
  {
