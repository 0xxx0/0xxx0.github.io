...
function studioProbeHtml(){
  ...
  const compact=x=>({focusHref:x?.focusHref||null,axisState:x?.axisState||null,projection:x?.projection||null,mapRoot:x?.mapRoot||null,mapOpen:!!x?.mapOpen});
  (async()=>{
    const W=()=>f.contentWindow,D=()=>W().document;
    ...
    rec.startHref=W().FieldLensHost.focus().href;rec.start=compact(W().FieldLensHost.uiState());
