...
function foldBloomLiveMobileBehaviorProbeHtml(){
  ...
  (async()=>{
    try{
      ...
      const hexRect=hex?.getBoundingClientRect(),release=doc.getElementById('releaseBtn'),releaseRect=release?.getBoundingClientRect();
      rec.initial={mode:w.FoldBloomPlay.state().mode,instrument:html.dataset.fbInstrument,hexVisual:html.dataset.fbHexVisual,view:html.dataset.fbView,hexOn:hex?.classList.contains('on')||false,hexDisplay:hex?getComputedStyle(hex).display:'missing',hexRect:hexRect?[Math.round(hexRect.left),Math.round(hexRect.top),Math.round(hexRect.width),Math.round(hexRect.height)]:null,releaseRect:releaseRect?[Math.round(releaseRect.left),Math.round(releaseRect.top),Math.round(releaseRect.width),Math.round(releaseRect.height)]:null,hexClear:!!(hexRect&&releaseRect&&hexRect.height>0&&hexRect.bottom<releaseRect.top-4),introOn:intro?.classList.contains('on')||false,introHidden:!!intro?.hidden};
      ...
      const pass=rec.initial.mode==='PUZZLE'&&rec.initial.instrument==='HEX'&&rec.initial.hexVisual==='two-rings'&&rec.initial.view==='ride'&&rec.initial.hexOn&&rec.initial.hexDisplay!=='none'&&rec.initial.hexClear&&!rec.initial.introOn&&rec.initial.introHidden&&rec.open.drawer&&rec.open.html&&rec.open.body&&rec.open.dismiss!=='none'&&/pan-y/.test(rec.open.htmlTouch)&&/pan-y/.test(rec.open.bodyTouch)&&rec.open.topId==='closeSettings'&&rec.drawerControl==='TRANCE'&&rec.backdropClosed&&rec.escapeClosed&&rec.xClosed&&rec.vibe.before!==rec.vibe.after&&rec.still.before==='ride'&&rec.still.mid==='still'&&rec.still.after==='ride'&&rec.overflow<=1;
      done(pass,rec);
...
const CASES=[
  ...
  },
  {
    ...
    route:'/fold-bloom/live/',
    options:{width:430,height:900,budget:9000},
    check:dom=>/LIVE 0\.13/i.test(dom)&&dom.includes('data-fold-bloom-live="ready"')&&dom.includes('data-fold-bloom-pov="embodied-v0.4"')&&dom.includes('data-fold-bloom-macro-drop="v0.2"')&&dom.includes('data-fold-bloom-idle-law="witness-v0.1"')&&((dom.includes('data-fold-bloom-idle="on"')&&dom.includes('data-fold-bloom-autopilot="on"'))||(dom.includes('data-fold-bloom-idle="off"')&&dom.includes('data-fold-bloom-autopilot="off"')))&&dom.includes('data-fold-bloom-landmarks="0"')&&dom.includes('data-fold-bloom-layer="IMMERSION"')&&dom.includes('id="demoBtn"')&&dom.includes('id="autoBtn"')&&dom.includes('id="publicDemoBtn"')&&/LOAD AUDIO EXAMPLE|PLAY AUDIO EXAMPLE/.test(dom)&&dom.includes('id="centerMassBtn"')&&/TRY CENTER MASS REMOTE/.test(dom)&&/RIDE FIELD COURSE/.test(dom)&&dom.includes('id="vaultSelect"')&&dom.includes('data-layer-mode="SOURCE"')&&dom.includes('data-layer-mode="MAP"')&&dom.includes('data-layer-mode="IMMERSION"')&&/SOURCE → MAP → IMMERSION/i.test(dom)&&dom.includes('id="call"')&&dom.includes('id="arc"')&&dom.includes('id="route"')&&dom.includes('id="trackFile"')&&dom.includes('id="vibeQuick"')&&dom.includes('id="menuDismiss"')&&dom.includes('id="lyric"')&&dom.includes('id="textBtn"')&&dom.includes('id="solidTune"')&&dom.includes('id="immersionTune"')&&dom.includes('id="anticipationTune"')&&dom.includes('id="motionGainTune"')&&dom.includes('id="dropGainTune"')&&dom.includes('id="textSyncTune"')&&dom.includes('data-xp-preset="DRIVE"')&&/data-fold-bloom-ride-profile="[^"]+"/.test(dom)&&/AUTOPILOT|TAKE OVER/.test(dom)&&dom.includes('data-fold-bloom-play="FOLD_BLOOM_PLAY_0.5.7"')&&/RIDE FIELD COURSE/i.test(dom)&&/PLAY MODES · OPTIONAL/i.test(dom)&&/VIBE · NORMAL/i.test(dom)&&/△ TRIANGLE|○ CIRCLE|□ SQUARE/.test(dom)&&/FIELD COURSE/.test(dom)&&dom.includes('data-trackfield-source="FIELD_PRACTICE"')&&/data-trackfield-motion="(?!NONE)[^"]+"/.test(dom)&&/data-fold-bloom-perf="[^"]+"/.test(dom)  },
  {
    name:'FOLD BLOOM LIVE mobile controls clear',
    route:'/fold-bloom/live/?play=PUZZLE',
    options:{width:430,height:900,budget:9000},
    check:dom=>dom.includes('data-fold-bloom-play="FOLD_BLOOM_PLAY_0.5.7"')&&dom.includes('data-fold-bloom-play-loop="FOLD_BLOOM_PLAY_LOOP_0.1"')&&dom.includes('data-fb-play-mode="PUZZLE"')&&dom.includes('data-fb-loop-archetype="FORM_CHANGE"')&&dom.includes('data-fb-surface="active"')&&dom.includes('data-fb-hex-visual="two-rings"')&&dom.includes('data-fb-view="ride"')&&dom.includes('data-fb-primary-controls="clear"')&&!/id="intro"[^>]*class="panel on"/.test(dom)&&/△ TRIANGLE|○ CIRCLE|□ SQUARE/.test(dom)
  },
  {
    ...
    check:dom=>/id="probeResult">PASS /.test(dom)&&/"instrument":"HEX"/.test(dom)&&/"hexVisual":"two-rings"/.test(dom)&&/"view":"ride"/.test(dom)&&/"hexDisplay":"block"/.test(dom)&&/"hexClear":true/.test(dom)&&/"drawerControl":"TRANCE"/.test(dom)&&/"topId":"closeSettings"/.test(dom)&&/"backdropClosed":true/.test(dom)&&/"escapeClosed":true/.test(dom)&&/"xClosed":true/.test(dom)&&/"mid":"still"/.test(dom)&&/"after":"ride"/.test(dom)&&/"overflow":0/.test(dom)
  },
  {
