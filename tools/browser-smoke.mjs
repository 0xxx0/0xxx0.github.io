...
function foldBloomLiveMobileBehaviorProbeHtml(){
  ...
  (async()=>{
    try{
      ...
      rec.open={drawer:settings.classList.contains('on'),html:html.classList.contains('fbMenuOpen'),body:body.classList.contains('fbMenuOpen'),dismiss:getComputedStyle(dismiss).display,htmlTouch:getComputedStyle(html).touchAction,bodyTouch:getComputedStyle(body).touchAction,topId:top?.id||top?.closest?.('button')?.id||null};
      ...
      const pass=rec.initial.mode==='PUZZLE'&&rec.initial.instrument==='HEX'&&rec.initial.hexVisual==='two-rings'&&rec.initial.view==='still'&&rec.initial.hexOn&&!rec.initial.introOn&&rec.initial.introHidden&&rec.open.drawer&&rec.open.html&&rec.open.body&&rec.open.dismiss!=='none'&&/pan-y/.test(rec.open.htmlTouch)&&/pan-y/.test(rec.open.bodyTouch)&&rec.open.topId==='closeSettings'&&rec.drawerControl==='TRANCE'&&rec.backdropClosed&&rec.escapeClosed&&rec.xClosed&&rec.vibe.before!==rec.vibe.after&&rec.still.before==='still'&&rec.still.mid==='ride'&&rec.still.after==='still'&&rec.overflow<=1;
      done(pass,rec);
...
const CASES=[
  ...
  },
  {
    ...
    options:{width:430,height:900,budget:9000},
    check:dom=>/LIVE 0\.13/i.test(dom)&&dom.includes('data-fold-bloom-live="ready"')&&dom.includes('data-fold-bloom-pov="embodied-v0.4"')&&dom.includes('data-fold-bloom-macro-drop="v0.2"')&&dom.includes('data-fold-bloom-idle-law="witness-v0.1"')&&((dom.includes('data-fold-bloom-idle="on"')&&dom.includes('data-fold-bloom-autopilot="on"'))||(dom.includes('data-fold-bloom-idle="off"')&&dom.includes('data-fold-bloom-autopilot="off"')))&&dom.includes('data-fold-bloom-landmarks="0"')&&dom.includes('data-fold-bloom-layer="IMMERSION"')&&dom.includes('id="demoBtn"')&&dom.includes('id="autoBtn"')&&dom.includes('id="publicDemoBtn"')&&/LOAD AUDIO EXAMPLE|PLAY AUDIO EXAMPLE/.test(dom)&&dom.includes('id="centerMassBtn"')&&/TRY CENTER MASS REMOTE/.test(dom)&&/PLAY FIELD COURSE/.test(dom)&&dom.includes('id="vaultSelect"')&&dom.includes('data-layer-mode="SOURCE"')&&dom.includes('data-layer-mode="MAP"')&&dom.includes('data-layer-mode="IMMERSION"')&&/SOURCE → MAP → IMMERSION/i.test(dom)&&dom.includes('id="call"')&&dom.includes('id="arc"')&&dom.includes('id="route"')&&dom.includes('id="trackFile"')&&dom.includes('id="vibeQuick"')&&dom.includes('id="menuDismiss"')&&dom.includes('id="lyric"')&&dom.includes('id="textBtn"')&&dom.includes('id="solidTune"')&&dom.includes('id="immersionTune"')&&dom.includes('id="anticipationTune"')&&dom.includes('id="motionGainTune"')&&dom.includes('id="dropGainTune"')&&dom.includes('id="textSyncTune"')&&dom.includes('data-xp-preset="DRIVE"')&&/data-fold-bloom-ride-profile="[^"]+"/.test(dom)&&/AUTOPILOT|TAKE OVER/.test(dom)&&dom.includes('data-fold-bloom-play="FOLD_BLOOM_PLAY_0.5.4"')&&/TURN → MATCH SHAPE → RELEASE/i.test(dom)&&/△ TRIANGLE|○ CIRCLE|□ SQUARE/.test(dom)&&/FIELD COURSE/.test(dom)&&dom.includes('data-trackfield-source="FIELD_PRACTICE"')&&/data-trackfield-motion="(?!NONE)[^"]+"/.test(dom)&&/data-fold-bloom-perf="[^"]+"/.test(dom)  },
  {
    name:'FOLD BLOOM LIVE mobile controls clear',
    route:'/fold-bloom/live/?play=PUZZLE',
    options:{width:430,height:900,budget:9000},
    check:dom=>dom.includes('data-fold-bloom-play="FOLD_BLOOM_PLAY_0.5.4"')&&dom.includes('data-fb-play-mode="PUZZLE"')&&dom.includes('data-fb-surface="active"')&&dom.includes('data-fb-hex-visual="two-rings"')&&dom.includes('data-fb-view="still"')&&dom.includes('data-fb-primary-controls="clear"')&&!/id="intro"[^>]*class="panel on"/.test(dom)&&/△ TRIANGLE|○ CIRCLE|□ SQUARE/.test(dom)
  },
  {
    ...
    check:dom=>/id="probeResult">PASS /.test(dom)&&/"instrument":"HEX"/.test(dom)&&/"hexVisual":"two-rings"/.test(dom)&&/"view":"still"/.test(dom)&&/"drawerControl":"TRANCE"/.test(dom)&&/"backdropClosed":true/.test(dom)&&/"escapeClosed":true/.test(dom)&&/"xClosed":true/.test(dom)&&/"mid":"ride"/.test(dom)&&/"after":"still"/.test(dom)&&/"overflow":0/.test(dom)
  },
  {
    name:'FOLD BLOOM LIVE Two Dial embodied mobile',
    route:'/fold-bloom/live/?play=DUET',
    options:{width:430,height:900,budget:9000},
    check:dom=>dom.includes('data-fold-bloom-play="FOLD_BLOOM_PLAY_0.5.4"')&&dom.includes('data-fb-play-mode="DUET"')&&dom.includes('data-fb-instrument="TWO_DIAL"')&&dom.includes('data-fb-primary-controls="clear"')&&/OUTER \/ DIAL A · ROAD/i.test(dom)&&/INNER \/ DIAL B · RELATION/i.test(dom)&&/LANDSCAPE ↔/i.test(dom)
  },
  {
