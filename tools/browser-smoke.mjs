...
function humanPortSpecimenProbeHtml(){
  ...
  async function intakeRoundTrip(tag,beforeFocus=null){
    ...
    D().getElementById('acceptSafeBtn').click();await sleep(80);D().getElementById('applyBtn').click();
    ...
    await waitFor(()=>D().getElementById('objectBar')?.classList.contains('on'),10000,'port reentry');
    await sleep(180);
  ...
  (async()=>{
    ...
    await waitFor(()=>session()?.focus?.address&&session().focus.address!=='((req,res)=>{
