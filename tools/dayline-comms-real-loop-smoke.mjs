...
function probe(){
  ...
  (async()=>{
    ...
    rec.witness=true;
    const complete=await wait(()=>[...D().querySelectorAll('#moves button')].find(b=>/COMPLETE/.test(b.textContent)),4000,'COMPLETE');
    complete.click();
    await wait(()=>W().DaylineConfluence.snapshot().tasks.find(t=>t.id===task.id)?.status==='done',4000,'Dayline completed');
    rec.completed=true;
    ...
    rec.focusReleased=W().DaylineConfluence.snapshot().state.selected!==task.id;
    ...
    const ok=exact&&rec.ran&&rec.witness&&rec.completed&&rec.focusReleased&&rec.offer.schema==='atlas-dayline-source-return/v0.1'&&rec.offer.authority==='OFFER_ONLY'&&rec.offer.task?.status==='done'&&rec.offer.proposed_native_state==='COVERED'&&rec.beforeAccept==='OPEN'&&rec.afterAccept==='COVERED'&&rec.offerConsumed&&rec.overflow<=1;
    done(ok,{...rec,exact});
