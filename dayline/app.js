...
function chooseFrame(){
 ...
 const selected=taskById(day.state.selected);if(selected&&(selected.status!=='done'||selected.sourceLink))return{kind:'task',task:selected};
...
function sourceEvidence(f){
 const src=f?.kind==='handoff'?f.handoff?.source:f?.kind==='task'?f.task?.sourceLink?.source:null;
 const raw=src?.evidence_signal||'CLEAR',token=window.FieldSignal?.normalize(raw)||'CLEAR';
 return{token,boundary:String(src?.proof_boundary||''),owner:String(src?.evidence_owner||'')};
}
function render(){
