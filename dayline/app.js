(()=>{'use strict';
const STORE='poly-atlas-dayline-branch-i-public-v1',RETURN_STORE='poly-atlas-dayline-branch-i-public-last-return',HANDOFF='atlas.dayline.handoff.v01';
...
function addTask(input={}){const title=String(input.title||'').trim();if(!title)throw Error('title required');const lo=hm(day.meta.dayStart),hi=hm(day.meta.dayEnd),now=clamp(hm(day.state.now),lo,Math.max(lo,hi-5));let id='t:work-'+slug(title),n=2;while(taskById(id))id='t:work-'+slug(title)+'-'+n++;
 const earliest=String(input.earliest||mh(now)),latest=String(input.latest||day.meta.dayEnd),t={id,title,contexts:Array.isArray(input.contexts)&&input.contexts.length?input.contexts:[...day.state.contexts],duration:clamp(Number(input.duration)||25,5,180),value:clamp(Number(input.value)||4,1,5),earliest,latest:hm(latest)>hm(earliest)?latest:day.meta.dayEnd,setup:clamp(Number(input.setup)||1,0,5),depends:Array.isArray(input.depends)?input.depends:[],status:'open',sourceClass:input.sourceClass||'USER',provenance:String(input.provenance||'dayline-confluence capture'),notes:String(input.notes||''),fieldRef:input.fieldRef||input.sourceRef||null};
 day.tasks.push(t);day.state.selected=t.id;event('CREATE',t.id,t.fieldRef||t.provenance);saveDay();return t}
function applyHandoff(){if(!handoff)return;const h=handoff,p=h.payload||{};if(h.kind==='CONTEXT'){const xs=(p.contexts||[]).map(String).filter(Boolean),prefix=String(p.replacePrefix||''),base=day.state.contexts.filter(x=>!prefix||!String(x).startsWith(prefix));day.state.contexts=[...new Set([...base,...xs])];event('CONTEXT_HANDOFF',String(p.sourceRef||h.source?.route||''),xs.join(','));saveDay()}
 else if(h.kind==='TASK'){const t=p.task||{};addTask({...t,sourceClass:'IMPORTED',fieldRef:t.sourceRef||h.source?.route||null,provenance:t.provenance||('explicit handoff · '+(h.source?.route||'source'))})}
...
function runTask(id){if(day.state.route.includes(id))return;if(day.state.route.length>=3){toast('RUN is full · 3/3');return}day.state.route.push(id);day.state.selected=id;event('ACT',id,'dayline-confluence');saveDay();render()}
function completeTask(id){const t=taskById(id);if(!t)return;t.status='done';day.state.route=day.state.route.filter(x=>x!==id);event('DONE',id);saveDay();render();toast('Marked done')}
function reopenTask(id){const t=taskById(id);if(!t)return;t.status='open';day.state.selected=id;event('REOPEN',id);saveDay();render()}
...
function syncNow(){const prev=day.state.now,next=mh(deviceMinute(day));day.state.now=next;event('TIME_SYNC','',prev+' → '+next);saveDay();render();toast('NOW '+next)}
function recordWitness(){frame=chooseFrame();const v=frameView(frame),t=v.task,note=$('#witnessInput').value.trim();if(!t){toast('Accept/adopt an object first');return}if(!note)return;day.evidence.return.push({at:new Date().toISOString(),task:t.id,note,kind:'WITNESS'});event('EVIDENCE','return',t.id);saveDay();$('#witnessInput').value='';render();toast('Witness recorded')}
...
function returnPacket(){
 const before=clone(sessionStart),after=core(),delta=day.events.slice(sessionEventStart),changed=hash(before)!==hash(after)||delta.length>0,dev=mh(deviceMinute(day)),v=frameView(chooseFrame()),last=v.task?lastWitnessFor(v.task.id):null;
 return{kind:'poly-atlas-return-f',version:3,carrier:'dayline-confluence/v0.1',returnClass:changed?'WORLD_DELTA':'ORIENTATION_SNAPSHOT',worldKey:'dayline:'+localDateKey(),generatedAt:new Date().toISOString(),clockWitness:{canonicalNow:day.state.now,deviceLocalNow:dev,deltaMinutes:hm(day.state.now)-hm(dev)},focus:{kind:frame.kind,title:v.title,owner:v.owner,address:v.address,taskId:v.task?.id||null,sourceRoute:v.route||null},moves:lastMoves.map(x=>x.label),witness:last||null,before,after,afterChecksum:hash(after),eventIds:delta.map(e=>e.id),delta,evidence:clone(day.evidence),nextRoutes:(day.state.route||[]).map(id=>({id,title:taskById(id)?.title||id}))}
}
async function doReturn(){const packet=returnPacket(),txt=JSON.stringify(packet,null,2);localStorage.setItem(RETURN_STORE,txt);event('RETURN',packet.focus.taskId||'',packet.afterChecksum);saveDay();try{await navigator.clipboard.writeText(txt);toast('RETURN copied')}catch(_){prompt('Copy RETURN:',txt)}$('#returnClass').textContent=packet.returnClass;$('#returnMeta').textContent=packet.afterChecksum+' · '+packet.eventIds.length+' events · source '+(packet.focus.sourceRoute||packet.focus.owner);sessionStart=core();sessionEventStart=day.events.length;render()}
...
function render(){
 ...
 const stored=(()=>{try{return JSON.parse(localStorage.getItem(RETURN_STORE)||'null')}catch(_){return null}})();$('#returnClass').textContent=stored?.returnClass||'READY';renderDepth();document.body.dataset.daylineWorkfield='ready'
}
...
window.DaylineConfluence=Object.freeze({snapshot:()=>clone(day),returnPacket:()=>returnPacket(),acceptHandoff:()=>applyHandoff()});
boot();
})();