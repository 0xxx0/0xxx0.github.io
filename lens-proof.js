...
function ensureUI(){
  ...
  el.innerHTML='<style>#lens-proof-bench{position:fixed;z-index:2147483645;right:max(8px,env(safe-area-inset-right));top:max(8px,env(safe-area-inset-top));width:min(390px,calc(100vw - 16px));border:1px solid #39454d;background:#090d0ff7;color:#edf1ef;font:9px/1.4 ui-monospace,monospace;box-shadow:0 8px 30px #000b;display:none}#lens-proof-bench.on{display:block}#lens-proof-bench *{box-sizing:border-box}#lens-proof-bench button,#lens-proof-bench select{border:0;border-radius:0;background:#0f1317;color:#edf1ef;font:inherit;padding:8px}#lens-proof-bench button{cursor:pointer}#lens-proof-bench button:hover{background:#172027}#lens-proof-bench .pbHead{display:flex;justify-content:space-between;align-items:center;padding:8px;border-bottom:1px solid #29343a}#lens-proof-bench .pbEy{font-size:7px;letter-spacing:.14em;color:#98d49b}#lens-proof-bench .pbGrid{display:grid;grid-template-columns:1fr 1fr;gap:1px;background:#29343a}#lens-proof-bench .pbGrid>*{min-width:0}#lens-proof-bench .pbRun{display:grid;grid-template-columns:1fr 1fr 1fr;gap:1px;background:#29343a;border-top:1px solid #29343a}#lens-proof-bench .pbNote{padding:8px;color:#8b989d;border-top:1px solid #29343a}#lens-proof-bench .pbStat{padding:8px;color:#72bce7;border-top:1px solid #29343a;white-space:pre-wrap}#lens-proof-bench .hot{color:#ffd0bd}#lens-proof-bench .good{color:#98d49b}#lens-proof-bench .close{padding:4px 7px}</style><div class="pbHead"><div><div class="pbEy">REP-01 / LENS PROOF</div><b>PLAIN ↔ FOVEATED</b></div><button class="close">×</button></div><div class="pbGrid"><select data-p="task"><option value="T1_LOCATE">T1 / LOCATE</option><option value="T2_CONTEXT">T2 / CONTEXT</option><option value="T3_REFRACT_RETURN">T3 / REFRACT + RETURN</option></select><select data-p="condition"><option value="PLAIN">PLAIN</option><option value="FOVEATED">FOVEATED</option></select><select data-p="orientation"><option value="CLEAR">ORIENTATION / CLEAR</option><option value="MINOR">ORIENTATION / MINOR RESIDUE</option><option value="LOST">ORIENTATION / LOST</option></select><button data-p="start" class="hot">START</button></div><div class="pbNote" data-p="note">Choose a task and condition. Condition setup is excluded from timing.</div><div class="pbRun"><button data-p="done" class="good">DONE</button><button data-p="copy">COPY RECEIPTS</button><button data-p="clear">CLEAR LOCAL</button></div><div class="pbStat" data-p="stat">NO RUNS YET</div>';
