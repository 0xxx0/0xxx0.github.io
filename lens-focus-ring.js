...
function attach(api){
  ...
  function render(){
    ...
    const xs=s.lensStack||[];stackBtn.querySelector('span').textContent=String(xs.length);
  ...
  function open(){if(!origin)origin=api.uiState?.()||null;panel.classList.add('on');render()}
  ...
  sh.querySelector('.rise').onclick=()=>{api.rise?.();render()};
  sh.querySelector('.dive').onclick=()=>{api.dive?.();render()};
  sh.querySelector('.prev').onclick=()=>{api.peer?.(-1);render()};
  sh.querySelector('.next').onclick=()=>{api.peer?.(1);render()};
  stackBtn.onclick=()=>stack.classList.toggle('on');
  sh.querySelector('.return').onclick=()=>{if(origin)api.restore?.(origin);origin=null;render()};
  sh.querySelector('.studio').onclick=()=>api.openStudio?.();
...
window.LensFocusRing=Object.freeze({attach});
