...
function attach(api,options={}){
  ...
  if(options.showTrigger===false)trigger.hidden=true;
  ...
  function render(){
    ...
    const xs=s.lensStack||[],catalog=api.catalog?.()||[],support=window.LensState?.supportCatalog?LensState.supportCatalog(s,catalog):[];stackBtn.querySelector('span').textContent=String(xs.length);
    const active=xs.length?xs.map((x,i)=>'<div class="stackRow"><b>'+(i+1)+' · '+esc(x.lensId)+'</b><span>'+esc(x.kind)+' / '+esc(x.authority)+'</span></div>').join(''):'<div>NO COMPOSED VIEW LENSES</div>';
    const next=support.length?'<div class="supportHead">LAWFUL NEXT / DEAD STAYS VISIBLE</div>'+support.map(x=>'<div class="supportRow '+(x.support?'live':'dead')+'"><div><b>'+esc(x.descriptor?.lensId||'—')+'</b><small>'+(x.support?esc(x.currentContract)+' → '+esc(x.nextContract):esc((x.reasons||[]).join(' · ')))+'</small></div><button data-lens="'+esc(x.descriptor?.lensId||'')+'">'+(x.support?'APPLY':'DEAD')+'</button></div>').join(''):'';
    stack.innerHTML=active+next;
    stack.querySelectorAll('[data-lens]').forEach(b=>b.onclick=()=>{api.applyLens?.(b.dataset.lens);mark('LENS_'+b.dataset.lens);render()});
    const modes=api.projections?.()||DEFAULT_MODES;rim.innerHTML=modes.map(m=>'<button data-mode="'+m+'" class="'+(m===s.projection?'on':'')+'">'+(m==='STRUCTURE'?'STRUCT':m.replace('_LATEST',''))+'</button>').join('');
  ...
  sh.querySelector('.proof').onclick=()=>{mark('OPEN_PROOF');window.LensProof?.open?.()};
...
window.LensFocusRing=Object.freeze({attach:(api,opt)=>activeInstance=attach(api,opt),RECEIPT_KEY,readReceipt,open:()=>activeInstance?.open(),close:()=>activeInstance?.close(),render:()=>activeInstance?.render()});
